...
在 Mobile Test Console 中选择两个已完成的历史运行，按页面截图配对后做并排/滑杆对比；图片默认适配当前视口，页面通过连续滚动浏览并同步当前项。某一侧缺失的页面给出可定位提示。主目录与 worktree 作为两个路径项目，各自走现有多项目 Runtime，原图不搬家。
...
8. 首版对比区一次只渲染选中的配对项，图片使用 `width: 100%`，纵向小程序截图会按列宽放大并产生很长的页面；浏览下一项依赖左侧列表点击。
...
3. **对比视图**：主区域默认左右并排，可切换滑杆覆盖。图片按可用视口限高并保持原始比例，点击图片仍可查看原图。顶部展示两侧项目名、目录末段、任务时间、任务 ID。
4. **缺失页提示**：
   - 只在左侧：`仅左侧存在`
   - 只在右侧：`仅右侧存在`
...
7. **连续浏览**：所有配对页在同一个限高滚动区按顺序展示。向下滚动自然进入下一页，左侧列表同步高亮当前页；点击列表项平滑滚动到对应页面。
8. **扩大查看区域**：`SCREENSHOT COMPARE` 配置区默认展开，并支持一键折叠；折叠后保留标题和总页数。
10. **对比区域与图片尺寸自适应**：
    - 对比区域外框高度自适应撑满剩余视口空间并顶到底部，消除底部空白，配置区折叠与展开时均能自动贴合视口底部。
    - 左侧页面导航列表与右侧对比展示区填满容器高度并独立滚动。
    - 并排与滑杆对比的图片大小自适应容器尺寸，消除固定像素硬编码上限，在保持原始长宽比的同时完整呈现实测页面，原图入口保持可用。
...
- [ ] 桌面端对比图默认完整适配可用视口高度，不再按内容列宽无限放大；原图入口保留
- [ ] 对比区连续渲染全部配对页，向下滚动进入下一页并同步左侧当前项，点击左侧项可定位对应页面
- [ ] 配置区默认展开，折叠后对比结果上移并扩大首屏查看空间
- [ ] 对比区域外框高度自适应顶到底部，左侧页面列表与右侧连续流各自填满并独立滚动
- [ ] 对比图片大小自适应容器宽高，保持原始纵横比且不被硬编码高度截断
- [ ] 上下方向键切换上一页/下一页，首尾页保持在边界，表单控件内按键不触发页面切换
