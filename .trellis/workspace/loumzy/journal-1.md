...
## Session 26: 统一账号与页面项目备份
<!-- trellis-session: v=2 fp=d4bb0385278871aa -->
...
完成统一项目备份导出/合并导入，包含账号画像、页面参数、启动导航、操作断言和两类录制；兼容旧账号 JSON，40 MiB 限额、整包校验、部分失败幂等重试及双工作区入口。独立审查通过；全量488通过/14失败与隔离HEAD基线失败名称一致；lint/schema/独立打包及package通过，既有3类型错误和开源扫描误报保留。只读核对真实数据5账号/6账号录制/28页面/48页面录制且完整对象一致。任务保持开放以跟踪已记录的基线质量门禁。

### Git Commits

| Hash | Message |
|------|---------|
| `17bc6b8` | feat(mtc): 统一备份账号画像与页面启动参数 |
