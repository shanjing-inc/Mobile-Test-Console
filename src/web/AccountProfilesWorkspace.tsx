...
const providerLabels: Record<AccountProfileProvider, string> = {
  wechat: "微信登录",
  qq: "QQ 登录",
  taobao: "淘宝登录",
  huawei: "华为登录",
  "taobao-commerce": "淘宝商品授权",
};

const initialProvider = "wechat" satisfies AccountProfileProvider;
const initialIdentity = accountProfileIdentityForProvider(initialProvider);
...
export function AccountProfilesWorkspace({ devices, onMessage }: {
  ...
  const [accountLabel, setAccountLabel] = useState(initialIdentity.accountLabel);
  const [provider, setProvider] = useState<AccountProfileProvider>(initialProvider);
  ...
  const recordingDevices = useMemo(
    () => resolveAccountProfileRecordingDevices(availableDevices, provider),
    [availableDevices, provider],
  );
  ...
  const replayDevices = resolveAccountProfileReplayDevices(availableDevices, selectedProfile?.platform, replayEntry);
  ...
  const changeProvider = (nextProvider: AccountProfileProvider) => {
    const nextIdentity = resolveAccountProfileIdentityChange(provider, nextProvider, profileId, accountLabel);
  ...
  const startRecording = async () => {
    if (!recordingDeviceKey || !profileId.trim() || !accountLabel.trim()) {
    ...
    try {
      ...
      onMessage({ kind: "info", text: `已启动 ${providerLabels[provider]} 录制` });
      await load();
    } catch (error) {
  ...
  return <div className="account-workspace">
    <section className="section-panel account-recording-panel">
      ...
      <div className="account-recording-grid">
        ...
        <label className="field"><span>授权场景</span><select value={provider} onChange={event => changeProvider(event.target.value as AccountProfileProvider)}>{ACCOUNT_PROFILE_PROVIDERS.map(item => <option key={item} value={item}>{providerLabels[item]}</option>)}</select></label>
        ...
        <button className="primary-button account-command" type="button" disabled={pending || !recordingDeviceKey || Boolean(selectedDeviceRecording)} onClick={() => void startRecording()}>{pending ? <LoaderCircle className="spin" size={15} /> : <Radio size={15} />}{selectedDeviceRecording ? "设备录制中" : "开始录制"}</button>
      </div>
      {activeRecordings.length > 0 && <div className="account-recording-sessions">
        ...
        {activeRecordings.map(item => <div className="account-recording-session" key={item.recordingId}>
          ...
          <span className="account-recording-identity"><strong>{item.accountLabel}</strong><small>{item.profileId} · {providerLabels[item.provider]}</small></span>
      ...
      <section className="section-panel account-profile-list">
        ...
        {(data?.profiles.length ?? 0) === 0
          ...
          : <div className="account-profile-rows">{data?.profiles.map(item => <button type="button" className={item.profileId === selectedProfileId ? "active" : ""} key={item.profileId} onClick={() => { setSelectedProfileId(item.profileId); setReplayProviderSelection(""); setReplay(null); setSource(null); }}>
            <Fingerprint size={17} />
            <span><strong>{item.accountLabel}</strong><small>{item.profileId} · {item.providerEntries.map(entry => providerLabels[entry.provider]).join(" / ")}</small></span>
      ...
      <section className="section-panel account-profile-detail">
        ...
        {selectedProfile ? <>
          <div className="account-meta-grid">
            <Meta label="Provider" value={selectedProfile.providerEntries.map(item => providerLabels[item.provider]).join(" / ")} />
          ...
          <div className="account-actions">
            <label className="field"><span>回放 Provider</span><select value={replayProvider} onChange={event => setReplayProviderSelection(event.target.value as AccountProfileProvider)}>{selectedProfile.providerEntries.map(item => <option key={item.provider} value={item.provider}>{providerLabels[item.provider]}</option>)}</select></label>
            ...
            <button className="primary-button" type="button" disabled={pending || !replayProvider || !replayDeviceKey} onClick={() => void runReplay()}><Play size={15} />{replayProvider === "taobao-commerce" ? "回放授权" : "回放登录"}</button>
