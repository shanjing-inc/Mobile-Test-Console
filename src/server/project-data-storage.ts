...
export interface ProjectDataStorage {
  id: string;
  directory: string;
...
export interface ProjectDataStorageInput {
  configPath: string;
  storageId?: string;
  stateDir: string;
...
export async function resolveProjectDataStorage(
  input: ProjectDataStorageInput,
  namespace: "account-profiles" | "page-parameters",
): Promise<ProjectDataStorage> {
  ...
  return {
    id,
    directory: path.join(userDataDir, namespace, id),
